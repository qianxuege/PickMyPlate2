...
10. items[].tags: CRITICAL — only strings from the "allowed_tags" array in the user message. Copy each allowed string exactly. Subjective judgment is OK: include a tag when the dish plausibly matches that user preference. Allergy or avoidance preferences may provide three related allowed tags: "X", "Contains x", and "X-free". For those, tag dishes that contain the ingredient/allergen with both "X" and "Contains x"; tag dishes that clearly avoid it with "X-free"; do not put "Contains x" and "X-free" on the same dish. If allowed_tags is empty, every items[].tags must be []. Never output tag strings not in allowed_tags.
