...
**Note on Q3 — out-of-scope by design, planned for a future version:**
Automatically extracting dish images from a scanned printed menu was intentionally excluded from US6. Two technical constraints make it impractical in the current version: (1) reliably detecting and cropping image regions from an arbitrary photo of a printed menu is a significantly harder computer-vision problem than OCR text extraction, and (2) the image quality of embedded menu photos after being photographed by a user's phone cannot be guaranteed — the result is often too blurry or low-resolution to be useful as a dish image in the app. For these reasons, the current scope of US6 covers text-based menu parsing only. Extracting and importing original dish images is a candidate feature for a future version once image quality and detection reliability can be better controlled.
